...
#define MQTT_BROKER "tcp://<Raspberry Pi IP>:1883"  // MQTT 브로커 URL
#define CLIENT_ID "SmartHomePi"                      // 클라이언트 ID
#define MQTT_TOPIC "smartHome/controls"              // 구독할 토픽
...
void on_message(struct mosquitto *mosq, void *obj, const struct mosquitto_message *message) {
    ...
    if (strcmp(message->topic, MQTT_TOPIC) == 0) {
        if (strcmp(message->payload, "open curtain") == 0) {
            // 커튼 열기
            setServoAngle(CURTAIN_SERVO_PIN, 0);  // 각도 조정
            relayControl(CURTAIN_RELAY_PIN, LOW);  // 릴레이 제어
        } else if (strcmp(message->payload, "close curtain") == 0) {
            // 커튼 닫기
            setServoAngle(CURTAIN_SERVO_PIN, 90);  // 각도 조정
            relayControl(CURTAIN_RELAY_PIN, HIGH);  // 릴레이 제어
        } else if (strcmp(message->payload, "open window") == 0) {
            // 창문 열기
            setServoAngle(WINDOW_SERVO_PIN, 90);  // 창문 열기
            relayControl(WINDOW_RELAY_PIN, HIGH); // 릴레이 제어 (창문 열기)
        } else if (strcmp(message->payload, "close window") == 0) {
            // 창문 닫기
            setServoAngle(WINDOW_SERVO_PIN, 0);  // 창문 닫기
            relayControl(WINDOW_RELAY_PIN, LOW);  // 릴레이 제어 (창문 닫기)
        } else if (strcmp(message->payload, "turn on led1") == 0) {
            // led1 켜기
            relayControl(LIGHT_RELAY_PIN, HIGH);  // 첫 번째 조명 켜기
        } else if (strcmp(message->payload, "turn off led1") == 0) {
            // led1 끄기
            relayControl(LIGHT_RELAY_PIN, LOW);   // 첫 번째 조명 끄기
        } else if (strcmp(message->payload, "turn on led2") == 0) {
            // led2 켜기
            relayControl(LIGHT2_PIN, HIGH);  // 두 번째 조명 켜기
        } else if (strcmp(message->payload, "turn off led2") == 0) {
            // led2 끄기
            relayControl(LIGHT2_PIN, LOW);   // 두 번째 조명 끄기
        }
    }
...
int main() {
    ...
    // MQTT 브로커 연결
    int ret = mosquitto_connect(mosq, "127.0.1.1", 1883, 60);  // 브로커 IP와 포트
    if (ret != MOSQ_ERR_SUCCESS) {
        fprintf(stderr, "Unable to connect to broker. Error: %s\n", mosquitto_strerror(ret));
        return 1;
    }
