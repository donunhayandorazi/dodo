...
void controlByBluetooth(int client) {
    ...
    if (bytes_read > 0) {
        ...
        if (command == 1) {
            printf("App: Closing window\n");
            setServoAngle(WINDOW_SERVO_PIN, 0);  // 창문 닫기
            relayControl(WINDOW_RELAY_PIN, LOW); // 창문 닫기 (릴레이 사용)
        } else if (command == 2) {
            printf("App: Opening window\n");
            setServoAngle(WINDOW_SERVO_PIN, 90); // 창문 열기
            relayControl(WINDOW_RELAY_PIN, HIGH); // 창문 열기 (릴레이 사용)
        } else if (command == 3) {
            printf("App: Closing curtain\n");
            setServoAngle(CURTAIN_SERVO_PIN, 90); // 커튼 닫기
            relayControl(CURTAIN_RELAY_PIN, HIGH); // 커튼 닫기 (릴레이 사용)
        } else if (command == 4) {
            printf("App: Opening curtain\n");
            setServoAngle(CURTAIN_SERVO_PIN, 0); // 커튼 열기
            relayControl(CURTAIN_RELAY_PIN, LOW); // 커튼 열기 (릴레이 사용)
        } else if (command == 5) {
            printf("App: Turning on LED 1\n");
            relayControl(LIGHT_RELAY_PIN, HIGH);   // LED1 켜기
        } else if (command == 6) {
            printf("App: Turning off LED 1\n");
            relayControl(LIGHT2_PIN, LOW); // LED1 끄기
        } else if (command == 7) {
            printf("App: Turning on LED 2\n");
            relayControl(LIGHT_RELAY_PIN, HIGH);   // LED2 켜기
        } else if (command == 8) {
            printf(" App: Turning off LED 2\n");
            relayControl(LIGHT2_PIN, LOW); // LED2 끄기
        }
    }
}
...
int main(void) {
    ...
    // 풍력 에너지가 강한 경우 창문 닫기 (바람이 강한 경우가 우선적 수행)
